use leptos::{CollectView, component, create_effect, create_resource, create_rw_signal, create_signal, IntoView, MaybeSignal, Params, server, ServerFnError, SignalGet, SignalSet, SignalUpdate, SignalWith, Transition, view, WriteSignal};
...
#[server]
pub async fn get_chapter(url: String, number: u32) -> Result<Option<ChapterResource>, ServerFnError<CustomError>> {
    crate::server::services::get_db_connexion_wrapper(|conn| crate::server::services::chapter_service::get_chapter_by_id(conn, url, number))
}
...
pub fn ChapterPage() -> impl IntoView {
    ...
    let params = move || params.with(|params| params.as_ref().map(|params| (params.url.clone().unwrap_or_default(), params.number.clone().unwrap_or_default())).unwrap_or_default());
    let (show_en, set_show_en) = use_cookie_with_options::<bool, FromToStringCodec>("en", UseCookieOptions::default().max_age(2592000_000_i64)); // 30 days
    let (show_cn, set_show_cn) = use_cookie_with_options::<bool, FromToStringCodec>("cn", UseCookieOptions::default().max_age(2592000_000_i64)); // 30 days
    let (show_local_en, set_show_local_en, _delete_show_en) =
        use_storage_with_options::<bool, FromToStringCodec>(StorageType::Local, "en", UseStorageOptions::<bool, FromToStringCodec>::default().initial_value(true));
    let (show_local_cn, set_show_local_cn, _delete_show_cn) =
        use_storage_with_options::<bool, FromToStringCodec>(StorageType::Local, "cn", UseStorageOptions::<bool, FromToStringCodec>::default().initial_value(false));
    ...
    let (font_size, set_font_size) = use_cookie_with_options::<i16, FromToStringCodec>("font_size", UseCookieOptions::default().max_age(2592000_000_i64)); // 30 days
    let (line_height, set_line_height) = use_cookie_with_options::<i16, FromToStringCodec>("line_height", UseCookieOptions::default().max_age(2592000_000_i64)); // 30 days
    let (font_size_local, set_font_size_local, _) = use_storage_with_options::<i16, FromToStringCodec>(StorageType::Local, "font_size", UseStorageOptions::<i16, FromToStringCodec>::default().initial_value(16));
    let (line_height_local, set_line_height_local, _) = use_storage_with_options::<i16, FromToStringCodec>(StorageType::Local, "line_height", UseStorageOptions::<i16, FromToStringCodec>::default().initial_value(160));
    ...
    fn get_icon(show: Option<bool>) -> ButtonVariant {
        if show.map_or(false, |val| val) { ButtonVariant::Primary } else {ButtonVariant::Outlined}
    }

    let chapter_page = create_resource(
        params,
        |value| async move {
    ...
    view! {
        ...
        <Transition fallback=move || view!{<p>"Loading..."</p>}>
        {
            move || chapter_page.with(|result| match result {
                ...
                Some(Err(e)) => {
                    match e {
                        WrappedServerError(ce) => match ce {CustomError::NotFound => view! {<NotFoundPage/>}.into_view(), _ => view! {""}.into_view()},
                        _ => view! {""}.into_view()
                    }
                },
                Some(Ok(chapter_option)) => match chapter_option {
                    None => view! {""}.into_view(),
                    Some(chapter) => {
                                                ...
                                                words.clone().into_iter().enumerate().map(|(idx_word, word)| {
                                                    match word.def {
                                                        ...
                                                        Some(_def) => {
                                                            ...
                                                            #[cfg(not(feature = "ssr"))]
                                                            let tooltip_ref = tooltip.duplicate();
                                                            #[cfg(not(feature = "ssr"))]
...
pub fn show(tooltip: &Tooltip, id: String, py: String, def: String) {
    tooltip.create(&format!("#{}", id), format!(r#"<div class="tooltip dict">
        <div class="py">{}</div>
        <div class="def">{}</div>
    </div>"#, py, def), true);
    tooltip.show();
