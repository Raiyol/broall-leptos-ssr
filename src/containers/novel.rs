use leptos::{CollectView, component, create_resource, create_signal, IntoView, Params, server, ServerFnError, SignalGet, SignalGetUntracked, SignalWith, Transition, view};
use leptos::ServerFnError::WrappedServerError;
use leptos_meta::Title;
use leptos_router::{A, Params, use_params};
...
pub async fn get_novel(url: String) -> Result<NovelWithShortChapters, ServerFnError<CustomError>> {
    crate::server::services::get_db_connexion_wrapper(|conn| crate::server::services::novel_service::find_novel_by_url_with_chapters_info(conn, url))
}
...
pub fn NovelPage() -> impl IntoView {
    ...
    let url = move || params.with(|params| params.as_ref().map(|params| params.url.clone()).unwrap_or_default().unwrap_or_default());
    let novel_page = create_resource(
        url,
        |value| async move {
            get_novel(value).await
        },
    );
